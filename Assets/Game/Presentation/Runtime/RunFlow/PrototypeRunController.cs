...
namespace Game.Presentation.RunFlow
{
    public sealed class PrototypeRunController : MonoBehaviour
    {
        ...
        private CombatPrototypeController _combatView;
        ...
        private void BuildUi()
        {
            ...
            _rewardPanel.CardSelected += OnCardSelected;
            _rewardPanel.CardSkipped += OnCardSkipped;
        ...
        {
            RunState run = _runManager.State;
            Player player = run.Players[0];
            List<Creature> enemies = new List<Creature>(room.Encounter.EnemyIds.Length);
            for (int i = 0; i < room.Encounter.EnemyIds.Length; i++)
            {
                EnemyModel enemyModel = ModelDb.Get<EnemyModel>(room.Encounter.EnemyIds[i]);
                enemies.Add(new Creature(enemyModel, enemyModel.MaxHp, enemyModel.MaxHp));
            }

            CombatState combat = new CombatState(run, room.Encounter, new[] { player }, enemies);
            _combatManager = new CombatManager();
            _combatManager.CombatEnded += OnCombatEnded;
            GameObject combatGo = new GameObject("CombatPrototypeController");
            _combatView = combatGo.AddComponent<CombatPrototypeController>();
            _combatManager.SetUpCombat(combat);
            _combatView.Bind(_combatManager);
            _ = _combatManager.StartCombatAsync();
        }

        private void OnCombatEnded(CombatState combat)
        {
            ...
            if (_combatView != null)
            {
                Destroy(_combatView.gameObject);
                _combatView = null;
            }
        ...
        private void OnCardSelected(CardRewardChoice reward, int index)
        {
            reward.Select(index);
            reward.Resolve(_runManager.State, _runManager.State.Players[0]);
            _runManager.SaveRun();
        ...
        private void OnCardSkipped(CardRewardChoice reward)
        {
            reward.Skip();
            reward.Resolve(_runManager.State, _runManager.State.Players[0]);
            _runManager.SaveRun();
