...
public static class RetroFakeLitConversionService
{
    ...
    static void ConfigureMaterial(Material material, Material source, string key)
    {
        ...
        material.SetFloat("_SpecStrength", key.IndexOf("brass", StringComparison.OrdinalIgnoreCase) >= 0 ? 0.05f : 0.025f);
    ...
    static Color PresetTint(string key)
    {
        ...
        if (lowerKey.Contains("wood") || lowerKey.Contains("table") || lowerKey.Contains("bird") || lowerKey.Contains("chest") || lowerKey.Contains("elephant") || lowerKey.Contains("farmer"))
        {
            return new Color(0.78f, 0.62f, 0.42f, 1f);
        }

        if (lowerKey.Contains("brass") || lowerKey.Contains("candle"))
        {
            return new Color(0.86f, 0.68f, 0.36f, 1f);
        }

        if (lowerKey.Contains("cat") || lowerKey.Contains("concrete") || lowerKey.Contains("stone"))
        {
            return new Color(0.86f, 0.80f, 0.70f, 1f);
        }

        if (lowerKey.Contains("panda") || lowerKey.Contains("girl"))
        {
