...
- On 2026-05-23, the render console gained a `RetroFakeLitConvert` page in `Assets/_Project/Editor/Rendering/CardDungeonRenderPipelineConsoleWindow.cs`. It uses `Assets/_Project/Editor/Rendering/RetroFakeLitConversionService.cs` to convert either a source folder or current scene selection into RetroFakeLit prefabs, writes output prefabs under `Assets/Arts/Prefabs/RetroFakeLits/`, and writes generated materials under `Assets/_Project/Rendering/Materials/RetroFakeLitGenerated/`.
- `RetroFakeLitConvert` recursively scans `gltf/glb/fbx/obj/dae/blend` files, uses parent folder names when source filenames are generic like `scene`, sanitizes output prefab names for asset-safe paths, and writes success/failure details both to the page debug panel and Unity Console.
- Verified conversion test on 2026-05-23: folder `Assets/Arts/Models/gezinkte_spielkarte_km-o.811` produced `Assets/Arts/Prefabs/RetroFakeLits/gezinkte_spielkarte_km_o_811_RetroFakeLit.prefab` with material `Assets/_Project/Rendering/Materials/RetroFakeLitGenerated/M_RetroFakeLit_scene_card_colour_12f771db.mat` using texture `Assets/Arts/Models/gezinkte_spielkarte_km-o.811/textures/card-colour_baseColor.png`; scene object `Assets/Tests/render.unity/PineTree` was connected to `Assets/Arts/Prefabs/RetroFakeLits/PineTree_RetroFakeLit.prefab` and now uses generated RetroFakeLit materials for trunk/leaves.
...
- The render console is organized into pages: Overview, Render Scale/URP baseline, Phase 05 RetroFakeLit, RetroFakeLitConvert, Phase 07 Posterize LUT, Phase 08 Retro Composite, and Presets. Each control includes user-facing explanation text about what increasing or decreasing the parameter changes.
